...
The open questions above got answered by a reader on a phone, and the answer was that half the component did not work there.

On a narrow screen the scroller collapses behind a **Browse 2,683 videos** button, which slides the list in as a full-screen panel. The list itself scrolled by finger. The **Up 5** and **Down 5** buttons did not. Up was disabled from the moment the panel opened and never came back.

The sidebar resolves its scroll container once, in `onMount`, by walking up for the nearest ancestor with `overflow-y: auto` or `scroll`. Which reads as sound until you notice *when* it runs. On mobile, at mount, the panel is closed — and the panel body only becomes scrollable once it is open:
...
So the walk finds nothing on the way up, falls through to the inline desktop scroller, and the buttons spend the rest of the session driving an element the reader cannot see. The disabled state follows from the same mistake: `isAtTop` starts `true`, and the only thing that clears it is a `scroll` listener bound to that same wrong element. No scroll, no clear, no Up button.

The fix is to stop treating the container as fixed for the life of the component. It is re-resolved on click, and on a `MutationObserver` watching `data-mfp-open` — the attribute that flips the panel body to `overflow-y: auto` in the first place. The scroll listener moves with it. Desktop still lands on `.yt-pl-scroller` exactly as before.

Driving a real 390×844 touch viewport, which is the only way this was ever going to be settled:
...
Worth writing down, because both are convincing and both cost time.

**Playwright scrolls an element into view before clicking it.** The button bars are `position: sticky`, so they *look* pinned — but the bottom bar's layout position sits after all fifty items. The pre-click scroll drags that position into view, the panel snaps to the very end, and it reads exactly like the component overshooting. A programmatic `.click()` on the same button stepped a clean 318px, which is what gave it away. A focus-suppression fix written for the phantom was reverted.

**The dev toolbar was eating the taps.** It overlays the bottom-centre of every page, above the site's own chrome, so a tap aimed at the bottom bar hits the toolbar instead and does nothing at all. Taps two and three in a row silently failing is a persuasive impression of a broken button. `devToolbar: { enabled: false }` now, which is dev-only and changes nothing about the deployed site.

Neither was a defect in the product. Both were the instrument, not the subject — a good reminder that a browser drive proves the buttons work, and that the drive itself needs verifying before its verdict is worth anything.
